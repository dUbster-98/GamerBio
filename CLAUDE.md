...
| **서버 OS** | Raspberry Pi OS 64-bit |
| **백엔드** | ASP.NET 8 Web API / C# |
| **DB** | PostgreSQL |
...
- [ ] ASP.NET 8 Web API 구성
  - `/api/emotion` — PC DeepFace 수신
  - `/api/biosignal` — ESP32 생체 데이터 수신
- [ ] PostgreSQL 스키마 설계 및 연동
...
| **최종 구성** | DB(PostgreSQL)와 API(ASP.NET 8 + Blazor Server) 모두 systemd 네이티브로 통일 |
...
- **ASP.NET API**: `dotnet publish` 결과물을 `/opt/biomonitor`에 배치 후 커스텀 unit 파일(`biomonitor-api.service`) 작성, `After=network.target postgresql.service` + `Requires=postgresql.service`로 의존성 명시
...
*최종 업데이트: Blazor 확정 + MJPEG 스트림 서버 + SignalR 실시간 대시보드 아키텍처 확정 + RPi5 배포 전략(전체 systemd 네이티브, Docker 미사용) 확정*
